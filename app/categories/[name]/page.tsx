...
export default function CategorySinglePage({ params }: { params: Promise<{ name: string }> }) {
    const { name } = use(params)
    const { location, setShowLocationModal } = useLocation()
    const router = useRouter()
    ...
    const createSlug = (categoryName: string) => {
        return categoryName.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]/g, '')
    }

    // Step 1: Fetch all categories and find the ID by matching the slug
    useEffect(() => {
        const fetchCategoryBySlug = async () => {
            try {
                ...
                // Find the category by comparing slugs
                const currentCat = cats.find((c: any) =>
                    createSlug(c.name) === name
                )
                ...
                safeErrorLog("Error fetching categories", err)
    ...
    // Step 2: Clear state when category ID or location changes
    useEffect(() => {
        setServices([])
        setProducts([])
        setError(null)
    ...
    // Step 3: Fetch services and products using the category ID
    useEffect(() => {
        const fetchData = async () => {
            if (!categoryId || !location?.lat || !location?.lng) return
                    ...
                    <div className="max-w-7xl mx-auto text-center space-y-8">
                        ...
                        {/* <div className="flex gap-3 overflow-x-auto pb-4 scrollbar-hide -mx-4 px-4 sm:mx-0 sm:px-0 justify-center pt-8">
                            {categories.map((cat) => (
                                <button
                                    key={cat.id}
                                    onClick={() => router.push(`/categories/${createSlug(cat.name)}`)}
                                    className={`px-8 py-3.5 rounded-2xl text-sm font-black tracking-widest uppercase transition-all whitespace-nowrap border-2 ${createSlug(cat.name) === name
                                        ? 'bg-[#101242] border-[#101242] text-white shadow-xl shadow-[#101242]/20 scale-105'
                                        : 'bg-white border-white text-slate-500 hover:border-slate-100 hover:bg-slate-50'
                                        }`}
                                >
