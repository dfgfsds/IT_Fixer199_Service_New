  ...
  return (
    <Link href={`/${basePath}/${product.id}`} className="group bg-white rounded-2xl border border-border/50 overflow-hidden hover:shadow-xl hover:border-primary/30 shadow-[0px_8px_16px_rgba(27,30,36,0.1)] transition-all duration-300 block">
      ...
      <div className="p-3 sm:p-6 space-y-2 sm:space-y-4">
        <div>
          <p className="text-[8px] sm:text-sm font-bold text-gray-800 uppercase tracking-wide mb-0.5 sm:mb-1">
            {product.category}
          </p>
          <h3 className="text-sm sm:text-xl font-bold text-[#101242] text-foreground group-hover:text-[#101242] transition-colors line-clamp-1">
            {product.name}
          </h3>
        </div>
